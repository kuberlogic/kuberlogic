...
const databaseDeletePermission = "service:database:delete"
...
func DatabaseDeleteWrapper(srv Service, next DatabaseDeleteHandlerFunc) (fn DatabaseDeleteHandlerFunc) {
	return func(params DatabaseDeleteParams, principal *models.Principal) middleware.Responder {
		...
		if authorized, err := authProvider.Authorize(principal.Token, databaseDeletePermission, params.ServiceID); err != nil {
			msg := "auth bad request"
			log.Errorw(msg, "permission", databaseDeletePermission, "serviceId", params.ServiceID, "error", err)
			return NewDatabaseDeleteBadRequest().WithPayload(&models.Error{
				Message: msg,
			})
		} else if !authorized {
			log.Errorw("auth forbidden", "permission", databaseDeletePermission, "serviceId", params.ServiceID)
