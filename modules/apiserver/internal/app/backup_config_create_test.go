...
func TestBackupConfigCreateSecretsAlreadyExists(t *testing.T) {
	namespace := "-test-namespace-"
	expectedObj := &kuberlogicv1.KuberLogicBackupSchedule{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test",
	...
	secretResource := &v1.Secret{
		...
		Data: map[string][]byte{
			"aws-access-key-id":     []byte("-test-access-key-"),
			"aws-secret-access-key": []byte("secret-key"),
			"bucket":                []byte("test-bucket"),
			"bucket-scope-suffix":   []byte(""),
			"endpoint":              []byte("https://endpoint.com"),
	...
	srv := &Service{
		log:              &TestLog{t: t},
		clientset:        fake.NewSimpleClientset(secretResource),
		kuberlogicClient: tc.client,
	}
	...
	checkMessage := func(payload interface{}) {
		err := payload.(*models.Error)
		expected := `failed to create a backup config: secrets "test" already exists`
		if !strings.HasPrefix(err.Message, expected) {
			t.Error("error does not equal: actual vs expected",
				payload, expected)
		}
