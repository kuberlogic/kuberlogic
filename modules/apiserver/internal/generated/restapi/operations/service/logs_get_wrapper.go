...
const logsGetPermission = "service:logs"
...
func LogsGetWrapper(srv Service, next LogsGetHandlerFunc) (fn LogsGetHandlerFunc) {
	return func(params LogsGetParams, principal *models.Principal) middleware.Responder {
		...
		if authorized, err := authProvider.Authorize(principal.Token, logsGetPermission, params.ServiceID); err != nil {
			msg := "auth bad request"
			log.Errorw(msg, "permission", logsGetPermission, "serviceId", params.ServiceID, "error", err)
			return NewLogsGetBadRequest().WithPayload(&models.Error{
				Message: msg,
			})
		} else if !authorized {
			log.Errorw("auth forbidden", "permission", logsGetPermission, "serviceId", params.ServiceID)
