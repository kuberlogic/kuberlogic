...
func BackupConfigDeleteWrapper(srv Service, next BackupConfigDeleteHandlerFunc) (fn BackupConfigDeleteHandlerFunc) {
	return func(params BackupConfigDeleteParams, principal *models.Principal) middleware.Responder {
		...
		if authorized, err := authProvider.Authorize(principal.Token, security.BackupConfigDeletePermission, params.ServiceID); err != nil {
			msg := "auth bad request"
			log.Errorw(msg, "permission", security.BackupConfigDeletePermission, "serviceId", params.ServiceID, "error", err)
			return NewBackupConfigDeleteBadRequest().WithPayload(&models.Error{
				Message: msg,
			})
		} else if !authorized {
			log.Errorw("auth forbidden", "permission", security.BackupConfigDeletePermission, "serviceId", params.ServiceID)
		...
		posthogMsg := posthog.NewMessage("backup-config-delete")
		...
		if perr := posthogMsg.Create(); perr != nil {
			...
			return NewBackupConfigDeleteServiceUnavailable().WithPayload(&models.Error{
				Message: msg,
			})
		}
