...
const userDeletePermission = "service:user:delete"
...
func UserDeleteWrapper(srv Service, next UserDeleteHandlerFunc) (fn UserDeleteHandlerFunc) {
	return func(params UserDeleteParams, principal *models.Principal) middleware.Responder {
		...
		if authorized, err := authProvider.Authorize(principal.Token, userDeletePermission, params.ServiceID); err != nil {
			msg := "auth bad request"
			log.Errorw(msg, "permission", userDeletePermission, "serviceId", params.ServiceID, "error", err)
			return NewUserDeleteBadRequest().WithPayload(&models.Error{
				Message: msg,
			})
		} else if !authorized {
			log.Errorw("auth forbidden", "permission", userDeletePermission, "serviceId", params.ServiceID)
